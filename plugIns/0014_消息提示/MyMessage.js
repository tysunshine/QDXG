...
;(function (window, document) {
	var MyMessage = function (option) {
		...
		if (!messageBox) {
			...
			messageBox.id = 'MyMessageBox';
			messageBox.timer1 = messageBox.timer2 = null;
			setStyle(messageBox, {
				position: 'fixed',
			    left: '50%',
			    zIndex: '9999',
		...
		setStyle(iconBox, {
			...
			width: '40px',
			height: '40px',
			background: aType[data.type].color
		})
		...
		iconImg.src = aType[data.type].src;
		setStyle(iconImg, {
			display: 'block',
			margin: '0 auto'
		})
		...
		// 插入到body中
		if (!document.getElementById('MyMessageBox')) {
			document.body.appendChild(messageBox);
		}

		// 动画效果
		clearInterval(messageBox.timer1);
		clearInterval(messageBox.timer2);
		setTimeout(function () {
			setStyle(messageBox, {
				opacity: 1,
				top: '80px'
			...
			// 关闭消息弹窗
			messageBox.timer1 = setTimeout(function () {
				setStyle(messageBox, {
					opacity: 0,
					top: '40px'
				})
				// 溢出消息弹窗
				messageBox.timer2 = setTimeout(function () {
